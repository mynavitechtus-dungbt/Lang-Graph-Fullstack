...
async def idea_analyzer_node(
    ...
    # get raw content from model
    raw_content = await model.ainvoke(messages, config=cfg)
    content = raw_content.content
    reply = content if isinstance(content, str) else str(content)
    reply = reply.strip()
    logger.info("idea_analyzer_node Reply: %s", reply)

    # if no reply, return fallback message this is edge case
    if not reply:
        fallback = (
            "Please share one concrete idea "
            "(a project, product, or topic) you'd like to explore."
        )
        logger.warning(
            "idea_analyzer_node No reply, returning fallback message"
        )
        return {"messages": [
    ...
    if parsed is None:
        logger.error(
            "idea_analyzer stage router parse failed: %s",
            routed.get("parsing_error")
        )
        wf = "idea"
    else:
