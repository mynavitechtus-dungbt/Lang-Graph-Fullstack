"""CocoIndex embedding flow for this repo.

Run updates against this file, not ``main.py`` (LangGraph entry has no CocoIndex flows)::

    cocoindex update index.py
    cocoindex update index.py:CodeEmbedding --full-reprocess -f   # rebuild all chunks

``cocoindex update main`` targets ``main.py`` and will not run the CodeEmbedding flow here.
"""
...
# Root of this repo (directory containing index.py). Do not use "../.." — that is relative to
# CWD and pointed two levels above the project when index.py lives at repo root.
...
def _main() -> None:
    ...
    while True:
        ...
        for result in query_output.results:
            loc = result["filename"]
            lines = f"L{result['start']['line']}-L{result['end']['line']}"
            print(f"[{result['score']:.3f}] {loc} ({lines})")
            print(f"    {result['code']}")
            print("---")
        print()
